...
GPIO_LED_REVERSE = 23     # Feux de recul (blanc)

REAR_LIGHT_DIM = 0.2      # Intensité feux de position arrière (20%)
REAR_LIGHT_BRAKE = 1.0    # Intensité freinage (100%)

# ─── Télémétrie / statut ────────────────────────────────────────────────────────
# Alimentation du Pi (power bank 5V régulé) : pas de % possible (sortie régulée),
# seulement un flag de sous-tension remonté par le Pi lui-même (vcgencmd).
TELEMETRY_PORT = 5002       # Pi → PC : paquets JSON de télémétrie
...
def build_telemetry():
    """Construit le dict de télémétrie (sous-tension de l'alimentation du Pi)."""
    return {'pi_undervolt': pi_undervoltage()}
...
def telemetry_loop():
    """Envoie la télémétrie au controller et publie le statut local (écran OLED)."""
    interval = 1.0 / TELEMETRY_RATE_HZ
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    print(f"[TELEM] Télémétrie → UDP {TELEMETRY_PORT}")
